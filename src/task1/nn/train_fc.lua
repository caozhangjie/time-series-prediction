...
cmd:option('-test_batch_size',     2,               'test batch size')
...
cmd:option('-save',               "fc", 'save directory')
...
data0 = npyio.loadnpy("../data/train.npy")
...
test_data0 = npyio.loadnpy("../data/test.npy")
...
do
    ...
    for j = 1, batch_num, 1
    do
        train_data = data_random[{{(order_epoch[j]-1) * opt.batch_size+1, order_epoch[j] * opt.batch_size}, {1,20}}]
        train_label = data_random[{{(order_epoch[j]-1) * opt.batch_size+1, order_epoch[j] * opt.batch_size},{21}}]
        if opt.cuda then
    ...
    do
        test_data = test_data_all[{{(order_epoch[j]-1) * opt.test_batch_size+1, order_epoch[j] * opt.test_batch_size}, {1,20}}]        
        test_label = test_data_all[{{(order_epoch[j]-1) * opt.test_batch_size+1, order_epoch[j] * opt.test_batch_size},{21}}]
