...
cmd:option('-save',               "cnn", 'save directory')
...
do
    ...
    for j = 1, batch_num, 1
    do
        train_data = data_random[{{(order_epoch[j]-1) * opt.batch_size+1, order_epoch[j] * opt.batch_size}, {},{}}]:reshape(torch.LongStorage{opt.batch_size, 14, 20, 1})
    ...
    do
        test_data = test_data_all[{{(order_epoch[j]-1) * opt.test_batch_size+1, order_epoch[j] * opt.test_batch_size}, {}, {}}]:reshape(torch.LongStorage{opt.test_batch_size, 14, 20, 1})       
        test_label = all_labels[{{(order_epoch[j]-1) * opt.test_batch_size+1, order_epoch[j] * opt.test_batch_size}}]:reshape(opt.test_batch_size, 1)
        if opt.cuda then
